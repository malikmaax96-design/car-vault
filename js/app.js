/* ============================================================
   CAR VAULT — FULL SPA APPLICATION LOGIC
   Data stored in localStorage. No server/backend required.
   ============================================================ */
...
const DB = {
  get(key) {
    try { return JSON.parse(localStorage.getItem('cv_' + key)) || []; }
    catch(e) { return []; }
  },
  set(key, data) {
    localStorage.setItem('cv_' + key, JSON.stringify(data));
  },
  genId() {
...
function getAll(entity)      { return DB.get(entity); }
function getById(entity, id) { return DB.get(entity).find(x => x.id === id) || null; }

function save(entity, obj) {
  const list = DB.get(entity);
  if (!obj.id) obj.id = DB.genId();
...
function remove(entity, id) {
  DB.set(entity, DB.get(entity).filter(x => x.id !== id));
}
...
function renderDashboard() {
  const stock       = getAll('stock');
  const sales       = getAll('sales');
  const workshop    = getAll('workshop');
  const complaints  = getAll('complaints');
    ...
    <div class="kpi-grid">
      <div class="kpi-card amber">
        <div class="kpi-header">
          ...
          <div class="kpi-icon amber"><i class="fas fa-car"></i></div>
        </div>
...
function buildRecentSalesHTML(sales) {
  ...
  return `
    <table class="data-table" style="min-width:auto">
      <thead><tr>
        <th>Reg</th><th>Buyer</th><th>Date</th><th>Profit</th>
      </tr></thead>
      <tbody>
        ${sales.map(s => {
...
function buildAlertsHTML(workshop, complaints) {
  ...
  workshop.filter(j => j.status !== 'done').slice(0, 4).forEach(j => {
    alerts.push({ type: 'warning', icon: 'fa-wrench', title: `Workshop: ${j.vehicleReg || 'Unknown'}`, desc: j.workRequired || 'Repair work pending' });
  });
  ...
  if (!alerts.length) {
    return `<div class="table-empty"><span class="empty-icon">✅</span><p>All clear — no alerts!</p></div>`;
  }
...
function buildDonutChart(stock) {
  ...
  activeCharts.donut = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{ data: Object.values(counts), backgroundColor: ['#10b981','#f59e0b','#3b82f6','#8b5cf6'], borderWidth: 2, borderColor: '#111b2e' }]
    },
    options: {
  ...
  content.innerHTML = `
    ...
    <div class="filter-bar">
      ...
      ${['all','available','workshop','sold','reserved'].map(f => `
        <button class="filter-btn ${_stockFilter === f ? 'active' : ''}" onclick="setStockFilter('${f}')">
          ${capitalize(f)}
        </button>`
      ).join('')}
    </div>
    <div class="quick-stats" id="stockStats"></div>
    <div class="table-wrapper" id="stockTableWrap"></div>
  `;
...
function renderStockTable() {
  ...
  if (statsEl) {
    statsEl.innerHTML = `
      ...
      <div class="quick-stat">Stock Value: <strong>£${all.filter(v=>v.status!=='sold').reduce((s,v)=>s+(parseFloat(v.purchasePrice)||0),0).toLocaleString()}</strong></div>
    `;
  }
  ...
  if (!stock.length) {
    wrap.innerHTML = `<div class="table-empty"><span class="empty-icon">🚗</span><p>${_stockSearch || _stockFilter !== 'all' ? 'No vehicles match your filter.' : 'No vehicles in stock. Add your first vehicle!'}</p></div>`;
    return;
  }
          ...
          return `<tr>
            ...
            <td>
              <div class="table-actions">
                <button class="btn btn-secondary btn-sm btn-icon" onclick="openStockModal('${v.id}')" title="Edit">
                  <i class="fas fa-edit"></i>
                </button>
                <button class="btn btn-danger btn-sm btn-icon" onclick="deleteStock('${v.id}')" title="Delete">
...
function openStockModal(id = null) {
  const v = id ? getById('stock', id) : null;
    ...
    <form id="stockForm">
      <div class="form-grid">
        <div class="form-group">
          ...
          <input class="form-control" id="sf_reg" value="${v?.registration||''}" placeholder="e.g. AB12 CDE" required style="text-transform:uppercase">
        </div>
        <div class="form-group">
          <label>Make *</label>
          <input class="form-control" id="sf_make" value="${v?.make||''}" placeholder="e.g. BMW, Ford" required>
        </div>
        <div class="form-group">
          <label>Model *</label>
          <input class="form-control" id="sf_model" value="${v?.model||''}" placeholder="e.g. 3 Series" required>
        </div>
        <div class="form-group">
          <label>Year</label>
          <input class="form-control" id="sf_year" type="number" value="${v?.year||''}" placeholder="e.g. 2020" min="1980" max="2035">
        </div>
        <div class="form-group">
          <label>Colour</label>
          <input class="form-control" id="sf_colour" value="${v?.colour||''}" placeholder="e.g. Midnight Black">
        </div>
        <div class="form-group">
          <label>Mileage</label>
          <input class="form-control" id="sf_mileage" type="number" value="${v?.mileage||''}" placeholder="e.g. 45000">
        </div>
        <div class="form-group">
          <label>Purchase Price (£)</label>
          <input class="form-control" id="sf_buy" type="number" step="0.01" value="${v?.purchasePrice||''}" placeholder="e.g. 12000">
        </div>
        <div class="form-group">
          <label>Asking / Sale Price (£)</label>
          <input class="form-control" id="sf_sell" type="number" step="0.01" value="${v?.salePrice||''}" placeholder="e.g. 15500">
        </div>
        <div class="form-group">
          <label>Status</label>
        ...
        <div class="form-group">
          ...
          <input class="form-control" id="sf_date" type="date" value="${v?.dateAdded||today()}">
        </div>
        <div class="form-group full-width">
          <label>Notes</label>
          <textarea class="form-control" id="sf_notes" placeholder="Any additional notes...">${v?.notes||''}</textarea>
        </div>
      </div>
  ...
  content.innerHTML = `
    <div class="section-header">
      ...
      <div class="section-actions">
        <div class="search-bar">
          <i class="fas fa-search"></i>
          <input type="text" id="wsSearchInput" placeholder="Search reg, problem..." oninput="onWsSearch(this.value)">
        </div>
        <button class="btn btn-primary" onclick="openWorkshopModal()">
          <i class="fas fa-plus"></i> Log Job
        </button>
      </div>
    </div>
    <div class="filter-bar">
      ...
      ${['all','pending','in-progress','done'].map(f => `
        <button class="filter-btn ${_workshopFilter===f?'active':''}" onclick="setWsFilter('${f}')">
          ${capitalize(f.replace('-',' '))}
        </button>`
      ).join('')}
    </div>
    <div class="table-wrapper" id="wsTableWrap"></div>
  `;
  renderWorkshopTable();
}
...
function onWsSearch(val) {
  _workshopSearch = val.toLowerCase();
  renderWorkshopTable();
}

function renderWorkshopTable() {
  let jobs = getAll('workshop');
  ...
  if (_workshopSearch) jobs = jobs.filter(j =>
    ...
    (j.workRequired||'').toLowerCase().includes(_workshopSearch) ||
  ...
  if (!jobs.length) {
    wrap.innerHTML = `<div class="table-empty"><span class="empty-icon">🔧</span><p>No workshop jobs found.</p></div>`;
    return;
  }
  ...
  wrap.innerHTML = `
    <table class="data-table">
      <thead><tr>
        ...
        <th>Parts</th><th>Mechanic</th><th>Total Cost</th><th>Date</th><th>Status</th><th>Actions</th>
      </tr></thead>
      <tbody>
        ${jobs.map(j => {
...
function openWorkshopModal(id = null) {
  const j = id ? getById('workshop', id) : null;
  const stockRegs = getAll('stock').map(v => v.registration);
    ...
    <form id="wsForm">
      <div class="form-grid">
        <div class="form-group">
          ...
          <datalist id="wsRegList">${stockRegs.map(r=>`<option value="${r}">`).join('')}</datalist>
        </div>
        <div class="form-group">
          <label>LWN Number</label>
          <input class="form-control" id="ws_lwn" value="${j?.lwn||''}" placeholder="e.g. LWN001">
        </div>
        <div class="form-group full-width">
          <label>Mechanical Problem</label>
          <input class="form-control" id="ws_problem" value="${j?.mechanicalProblem||''}" placeholder="Describe the issue...">
        </div>
        <div class="form-group full-width">
          <label>Work Required</label>
          <textarea class="form-control" id="ws_work" placeholder="Work to be carried out...">${j?.workRequired||''}</textarea>
        </div>
        <div class="form-group full-width">
          <label>Parts Required</label>
          <input class="form-control" id="ws_parts" value="${j?.partsRequired||''}" placeholder="List parts needed...">
        </div>
        <div class="form-group">
          <label>Labour Cost (£)</label>
          <input class="form-control" id="ws_labour" type="number" step="0.01" value="${j?.labourCost||''}" placeholder="0">
        </div>
        <div class="form-group">
          <label>Parts Cost (£)</label>
          <input class="form-control" id="ws_parts_cost" type="number" step="0.01" value="${j?.partsCost||''}" placeholder="0">
        </div>
        <div class="form-group">
          <label>Mechanic / Inspector</label>
          <input class="form-control" id="ws_mechanic" value="${j?.mechanic||''}" placeholder="e.g. MUGHEERA">
        </div>
        <div class="form-group">
          <label>Status</label>
        ...
        <div class="form-group">
          ...
          <input class="form-control" id="ws_date_logged" type="date" value="${j?.dateLogged||today()}">
        </div>
        <div class="form-group">
          <label>Date Completed</label>
          <input class="form-control" id="ws_date_done" type="date" value="${j?.dateCompleted||''}">
        </div>
      </div>
      <div class="form-footer">
        <button type="button" class="btn btn-secondary" onclick="closeModal()">Cancel</button>
  ...
  content.innerHTML = `
    <div class="section-header">
      ...
      <div class="section-actions">
        <div class="search-bar">
          <i class="fas fa-search"></i>
          <input type="text" id="inspSearchInput" placeholder="Search reg, inspector..." oninput="onInspSearch(this.value)">
        </div>
        <button class="btn btn-primary" onclick="openInspModal()">
          <i class="fas fa-plus"></i> Log Inspection
        </button>
      </div>
    </div>
    <div class="table-wrapper" id="inspTableWrap"></div>
  `;
  renderInspTable();
}

function onInspSearch(val) {
  _inspSearch = val.toLowerCase();
  renderInspTable();
}

function renderInspTable() {
  let insps = getAll('inspections');
  ...
  if (!insps.length) {
    wrap.innerHTML = `<div class="table-empty"><span class="empty-icon">📋</span><p>No inspections recorded yet.</p></div>`;
    return;
  }
...
function openInspModal(id = null) {
  const insp = id ? getById('inspections', id) : null;
  const stockRegs = getAll('stock').map(v => v.registration);
    ...
    <form id="inspForm">
      <div class="form-grid">
        <div class="form-group">
          ...
          <datalist id="inspRegList">${stockRegs.map(r=>`<option value="${r}">`).join('')}</datalist>
        </div>
        <div class="form-group">
          <label>Inspection Date</label>
          <input class="form-control" id="insp_date" type="date" value="${insp?.inspectionDate||today()}">
        </div>
        <div class="form-group">
          <label>Inspector Name</label>
          <input class="form-control" id="insp_inspector" value="${insp?.inspector||''}" placeholder="e.g. MUGHEERA">
        </div>
        <div class="form-group">
          <label>Result</label>
        ...
        <div class="form-group full-width">
          ...
          <textarea class="form-control" id="insp_findings" placeholder="Describe inspection findings...">${insp?.findings||''}</textarea>
        </div>
      </div>
      <div class="form-footer">
        <button type="button" class="btn btn-secondary" onclick="closeModal()">Cancel</button>
  ...
  content.innerHTML = `
    <div class="section-header">
      ...
      <div class="section-actions">
        <div class="search-bar">
          <i class="fas fa-search"></i>
          <input type="text" id="salesSearchInput" placeholder="Search buyer, vehicle..." oninput="onSalesSearch(this.value)">
        </div>
    ...
    <div class="quick-stats">
      <div class="quick-stat">Total Sales: <strong>${sales.length}</strong></div>
      <div class="quick-stat">Total Revenue: <strong>£${totalRev.toLocaleString()}</strong></div>
      <div class="quick-stat">Total Profit: <strong style="color:var(--green)">£${totalProfit.toLocaleString()}</strong></div>
      <div class="quick-stat">Avg Profit: <strong>£${sales.length ? Math.round(totalProfit/sales.length).toLocaleString() : 0}</strong></div>
    </div>
    <div class="table-wrapper" id="salesTableWrap"></div>
  `;
  renderSalesTable();
}

function onSalesSearch(val) {
  _salesSearch = val.toLowerCase();
  renderSalesTable();
}

function renderSalesTable() {
  let sales = getAll('sales');
  ...
  if (!sales.length) {
    wrap.innerHTML = `<div class="table-empty"><span class="empty-icon">💰</span><p>No sales recorded yet.</p></div>`;
    return;
  }
...
function openSaleModal(id = null) {
  const s = id ? getById('sales', id) : null;
  const stockVehicles = getAll('stock');
  ...
  document.getElementById('modalBody').innerHTML = `
    <form id="saleForm">
      <div class="form-grid">
        <div class="form-group">
          <label>Vehicle Reg</label>
          <input class="form-control" id="sl_reg" value="${s?.vehicleReg||''}" style="text-transform:uppercase" placeholder="e.g. AB12 CDE"
            list="slRegList" oninput="autoFillSale()">
          <datalist id="slRegList">${stockVehicles.map(v=>`<option value="${v.registration}">`).join('')}</datalist>
        </div>
        <div class="form-group">
          <label>Vehicle Description</label>
          <input class="form-control" id="sl_desc" value="${s?.vehicleDesc||''}" placeholder="e.g. BMW 3 Series 2020">
        </div>
        <div class="form-group">
          <label>Buyer Name</label>
          <input class="form-control" id="sl_buyer" value="${s?.buyerName||''}" placeholder="Full name">
        </div>
        <div class="form-group">
          <label>Sale Date</label>
          <input class="form-control" id="sl_date" type="date" value="${s?.saleDate||today()}">
        </div>
        <div class="form-group">
          <label>Purchase Price (£)</label>
          <input class="form-control" id="sl_buy" type="number" step="0.01" value="${s?.purchasePrice||''}" placeholder="0" oninput="calcProfit()">
        </div>
        <div class="form-group">
          <label>Sale Price (£)</label>
          <input class="form-control" id="sl_sell" type="number" step="0.01" value="${s?.salePrice||''}" placeholder="0" oninput="calcProfit()">
        </div>
        <div class="form-group">
          <label>Profit (£) — Auto Calculated</label>
          <input class="form-control" id="sl_profit" type="number" step="0.01" value="${s?.profit||''}" readonly style="opacity:0.65;cursor:not-allowed">
        </div>
        <div class="form-group">
        ...
        <div class="form-group full-width">
          <label>Notes</label>
          <textarea class="form-control" id="sl_notes" placeholder="Any additional details...">${s?.notes||''}</textarea>
        </div>
      </div>
      <div class="form-footer">
        <button type="button" class="btn btn-secondary" onclick="closeModal()">Cancel</button>
...
function autoFillSale() {
  ...
  const v = getAll('stock').find(x => x.registration === reg);
  if (v) {
...
function saveSale(id) {
  ...
  // Mark vehicle as sold if it exists in stock
  if (!id && reg) {
    const v = getAll('stock').find(x => x.registration === reg);
    if (v && v.status !== 'sold') {
      v.status = 'sold';
  ...
  content.innerHTML = `
    <div class="section-header">
      ...
      <div class="section-actions">
        <div class="search-bar">
          <i class="fas fa-search"></i>
          <input type="text" id="compSearchInput" placeholder="Search customer, vehicle..." oninput="onCompSearch(this.value)">
        </div>
        <button class="btn btn-primary" onclick="openCompModal()">
    ...
    <div class="filter-bar">
      ...
      ${['all','open','in-progress','resolved'].map(f => `
        <button class="filter-btn ${_compFilter===f?'active':''}" onclick="setCompFilter('${f}')">
          ${capitalize(f.replace('-',' '))}
        </button>`
      ).join('')}
    </div>
    <div class="table-wrapper" id="compTableWrap"></div>
  `;
  renderCompTable();
}
...
function onCompSearch(val) {
  _compSearch = val.toLowerCase();
  renderCompTable();
}

function renderCompTable() {
  let comps = getAll('complaints');
  ...
  if (!comps.length) {
    wrap.innerHTML = `<div class="table-empty"><span class="empty-icon">😊</span><p>No complaints found — that's great!</p></div>`;
    return;
  }
...
function openCompModal(id = null) {
  const c = id ? getById('complaints', id) : null;
    ...
    <form id="compForm">
      <div class="form-grid">
        <div class="form-group">
          ...
          <input class="form-control" id="co_name" value="${c?.customerName||''}" placeholder="Full name">
        </div>
        <div class="form-group">
          <label>Vehicle Reg</label>
          <input class="form-control" id="co_reg" value="${c?.vehicleReg||''}" style="text-transform:uppercase" placeholder="e.g. AB12 CDE">
        </div>
        <div class="form-group">
          <label>Date Raised</label>
          <input class="form-control" id="co_date" type="date" value="${c?.dateRaised||today()}">
        </div>
        <div class="form-group">
          <label>Status</label>
        ...
        <div class="form-group full-width">
          ...
          <textarea class="form-control" id="co_issue" placeholder="Describe the complaint...">${c?.issue||''}</textarea>
        </div>
        <div class="form-group">
          <label>Date Resolved</label>
          <input class="form-control" id="co_resolved" type="date" value="${c?.dateResolved||''}">
        </div>
        <div class="form-group full-width">
          <label>Resolution Notes</label>
          <textarea class="form-control" id="co_resolution" placeholder="How was this resolved?">${c?.resolution||''}</textarea>
        </div>
      </div>
      <div class="form-footer">
        <button type="button" class="btn btn-secondary" onclick="closeModal()">Cancel</button>
...
function renderReports() {
  ...
  content.innerHTML = `
    <div class="section-header">
      ...
      <div class="section-actions">
        <button class="btn btn-secondary" onclick="exportCSV('sales')" title="Export Sales to CSV">
          <i class="fas fa-file-csv"></i> Sales CSV
        </button>
        <button class="btn btn-secondary" onclick="exportCSV('stock')" title="Export Stock to CSV">
          <i class="fas fa-file-csv"></i> Stock CSV
        </button>
    ...
    <div class="kpi-grid">
      ...
      <div class="kpi-card blue">
        ...
        <div class="kpi-sub">${stock.filter(v=>v.status!=='sold').length} vehicles in stock</div>
      </div>
      <div class="kpi-card red">
...
function buildMakesChart(stock) {
  ...
  if (!ctx) return;
  const counts = {};
  stock.filter(v => v.status === 'sold').forEach(v => {
    const m = v.make || 'Unknown';
    counts[m] = (counts[m] || 0) + 1;
  });
  const sorted = Object.entries(counts).sort((a,b)=>b[1]-a[1]).slice(0,8);
  if (!sorted.length) {
    ctx.closest('.chart-container').innerHTML = '<div style="text-align:center;padding:40px;color:var(--text-muted)">No sold vehicles yet</div>';
    return;
  }
  ...
  activeCharts.makes = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{ label: 'Cars Sold', data: sorted.map(x=>x[1]), backgroundColor: colors, borderRadius: 7 }]
    },
    options: {
      responsive: true, maintainAspectRatio: false,
...
function exportCSV(entity) {
  ...
  toast(`${capitalize(entity)} data exported!`, 'success');
}
...
function exportAllData() {
  const data = {
    ...
    version:      '1.0',
    stock:        getAll('stock'),
    workshop:     getAll('workshop'),
    inspections:  getAll('inspections'),
    sales:        getAll('sales'),
...
function handleImport(e) {
  ...
  reader.onload = ev => {
    try {
      ...
      ['stock','workshop','inspections','sales','complaints'].forEach(key => {
        if (Array.isArray(data[key])) DB.set(key, data[key]);
      });
...
function updateNavBadges() {
  const pendingJobs = getAll('workshop').filter(j => j.status !== 'done').length;
  const openComps   = getAll('complaints').filter(c => c.status === 'open').length;

  document.querySelectorAll('.nav-item[data-page]').forEach(el => {
    const existingBadge = el.querySelector('.nav-badge');
    if (existingBadge) existingBadge.remove();

    if (el.dataset.page === 'workshop' && pendingJobs > 0) {
      el.insertAdjacentHTML('beforeend', `<span class="nav-badge">${pendingJobs}</span>`);
    }
    if (el.dataset.page === 'complaints' && openComps > 0) {
      el.insertAdjacentHTML('beforeend', `<span class="nav-badge">${openComps}</span>`);
    }
  });
}
...
function seedDemoData() {
  if (getAll('stock').length > 0) return; // already populated

  const vehicles = [
    { id: DB.genId(), registration:'AB12CDE', make:'BMW',        model:'3 Series',   year:'2020', colour:'Midnight Black',  mileage:'45000', purchasePrice:'12000', salePrice:'16500', status:'available', dateAdded:'2026-03-01', notes:'' },
    { id: DB.genId(), registration:'EF34GHI', make:'Mercedes',   model:'C-Class',    year:'2019', colour:'Polar White',      mileage:'62000', purchasePrice:'14500', salePrice:'19000', status:'workshop',  dateAdded:'2026-03-05', notes:'' },
    { id: DB.genId(), registration:'JK56LMN', make:'Audi',       model:'A4',         year:'2021', colour:'Tango Red',        mileage:'28000', purchasePrice:'18000', salePrice:'24500', status:'sold',      dateAdded:'2026-02-10', notes:'' },
    { id: DB.genId(), registration:'OP78QRS', make:'Volkswagen', model:'Golf',       year:'2022', colour:'Indium Grey',      mileage:'15000', purchasePrice:'13500', salePrice:'17500', status:'available', dateAdded:'2026-03-15', notes:'' },
    { id: DB.genId(), registration:'TU90VWX', make:'Ford',       model:'Focus',      year:'2020', colour:'Electric Blue',    mileage:'38000', purchasePrice:'8500',  salePrice:'12000', status:'reserved',  dateAdded:'2026-03-20', notes:'' },
    { id: DB.genId(), registration:'YZ12ABC', make:'Toyota',     model:'Corolla',    year:'2021', colour:'Pearl White',      mileage:'22000', purchasePrice:'11000', salePrice:'15000', status:'sold',      dateAdded:'2026-02-22', notes:'' },
    { id: DB.genId(), registration:'CD34EFG', make:'Honda',      model:'Civic',      year:'2022', colour:'Crystal Black',    mileage:'18000', purchasePrice:'13000', salePrice:'17000', status:'available', dateAdded:'2026-04-01', notes:'' },
    { id: DB.genId(), registration:'GH56IJK', make:'Vauxhall',   model:'Astra',      year:'2020', colour:'Satin Steel Grey', mileage:'42000', purchasePrice:'7800',  salePrice:'11500', status:'sold',      dateAdded:'2026-03-25', notes:'' },
  ];
  vehicles.forEach(v => save('stock', v));

  const workshopJobs = [
    { id:DB.genId(), vehicleReg:'EF34GHI', lwn:'LWN001', mechanicalProblem:'Steering noise on full lock', workRequired:'Replace steering rack', partsRequired:'Steering rack assembly', labourCost:'250', partsCost:'420', mechanic:'MUGHEERA', status:'in-progress', dateLogged:'2026-03-06', dateCompleted:'' },
    { id:DB.genId(), vehicleReg:'AB12CDE', lwn:'LWN002', mechanicalProblem:'Brake wear indicator light on', workRequired:'Replace front brake pads and discs', partsRequired:'Brake pads + discs set', labourCost:'120', partsCost:'95', mechanic:'MOE', status:'done', dateLogged:'2026-03-10', dateCompleted:'2026-03-11' },
    { id:DB.genId(), vehicleReg:'OP78QRS', lwn:'LWN003', mechanicalProblem:'Right front lever not working', workRequired:'Inspect and repair right front CV joint', partsRequired:'CV joint kit', labourCost:'180', partsCost:'85', mechanic:'MUGHEERA', status:'pending', dateLogged:'2026-04-02', dateCompleted:'' },
  ];
  workshopJobs.forEach(j => save('workshop', j));

  const inspections = [
    { id:DB.genId(), vehicleReg:'AB12CDE', inspectionDate:'2026-03-01', inspector:'MUGHEERA', alloyCondition:'Good', mechanicalCondition:'Fair', findings:'Minor brake wear noted. Front discs at minimum spec. Tyres good all round.', result:'advisory' },
    { id:DB.genId(), vehicleReg:'EF34GHI', inspectionDate:'2026-03-05', inspector:'MUGHEERA', alloyCondition:'Fair', mechanicalCondition:'Poor', findings:'Steering rack making noise under full lock. Needs urgent replacement before sale.', result:'fail' },
    { id:DB.genId(), vehicleReg:'JK56LMN', inspectionDate:'2026-02-10', inspector:'MOE',      alloyCondition:'Good', mechanicalCondition:'Good', findings:'Excellent condition throughout. Minor stone chips on bonnet, no bodywork concerns.', result:'pass' },
    { id:DB.genId(), vehicleReg:'OP78QRS', inspectionDate:'2026-03-16', inspector:'MOE',      alloyCondition:'Good', mechanicalCondition:'Fair', findings:'CV joint making clicking noise on right front. Alloys in good condition.', result:'advisory' },
  ];
  inspections.forEach(i => save('inspections', i));

  const salesData = [
    { id:DB.genId(), vehicleReg:'JK56LMN', vehicleDesc:'Audi A4 2021',       buyerName:'Ahmed Khan',    saleDate:'2026-03-15', purchasePrice:'18000', salePrice:'24500', profit:'6500', paymentMethod:'finance',       notes:'Part financed through Startline Motor Finance' },
    { id:DB.genId(), vehicleReg:'YZ12ABC', vehicleDesc:'Toyota Corolla 2021', buyerName:'Sarah Johnson', saleDate:'2026-03-22', purchasePrice:'11000', salePrice:'15000', profit:'4000', paymentMethod:'cash',          notes:'' },
    { id:DB.genId(), vehicleReg:'GH56IJK', vehicleDesc:'Vauxhall Astra 2020', buyerName:'Mohammed Ali',  saleDate:'2026-04-01', purchasePrice:'7800',  salePrice:'11500', profit:'3700', paymentMethod:'part-exchange', notes:'Part ex: Ford Fiesta 2018 valued at £4000' },
    { id:DB.genId(), vehicleReg:'CD99XYZ', vehicleDesc:'Range Rover Sport 2019', buyerName:'Lisa Brown', saleDate:'2026-02-20', purchasePrice:'28000', salePrice:'35000', profit:'7000', paymentMethod:'finance',       notes:'' },
    { id:DB.genId(), vehicleReg:'PQ88RST', vehicleDesc:'Mercedes A-Class 2022', buyerName:'Tariq Hussain', saleDate:'2026-04-05', purchasePrice:'16000', salePrice:'21000', profit:'5000', paymentMethod:'cash',       notes:'' },
  ];
  salesData.forEach(s => save('sales', s));

  const complaintsData = [
    { id:DB.genId(), customerName:'Ahmed Khan',  vehicleReg:'JK56LMN', dateRaised:'2026-03-20', issue:'Engine warning light (EML) came on 5 days after purchase', status:'open',        dateResolved:'', resolution:'' },
    { id:DB.genId(), customerName:'Lisa Brown',  vehicleReg:'CD99XYZ', dateRaised:'2026-02-28', issue:'Air conditioning not working. Car was advertised with working A/C', status:'resolved',  dateResolved:'2026-03-05', resolution:'Recharged A/C system at no cost to customer' },
  ];
  complaintsData.forEach(c => save('complaints', c));
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Load demo data, then navigate to dashboard
  seedDemoData();
  navigate('dashboard');
});
